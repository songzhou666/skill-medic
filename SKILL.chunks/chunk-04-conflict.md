...
> 阈值统一在 `medic_tools/run.py` 顶部"集中配置"区维护，调整必须同步本表、需求文档 §5.3 / §9.2 与 CHANGELOG。

| 冲突类型 | 静态命中阈值 | 严重度判定 |
|----------|-------------|------------|
| C1 | **仅同功能域比对**：交集 bigram ≥ 20（或 ≥ 8 且 Jaccard ≥ 0.12）；跨域不做 C1 判定 | 高：同质且评分差 ≥ 20；中：同质但评分差 < 20 |
| C2 | 非模板词交集 ≥ 4 且至少一方无反触发；**跨功能域对需更高信号：交集 ≥ 8 且 Jaccard ≥ 0.15** | 高：交集 ≥ 10；中：4~9 |
...
> **规模分级策略（§6.2.1，少则精、多则省）**：按活跃 Skill 数分四档，C1/C2 候选降噪只在**大档位**启用——
> - **S1 精细（≤20）**：不做 Top-K 降噪，保留全部真候选，逐冲突核对
> - **S2 标准（21~80）**：不做 Top-K 降噪，保留全部真候选，分批精析
> - **S3 摘要（81~300）**：C1/C2 候选按功能域分组后，每域每类型只保留相似度最高的 Top-15 组
>   （C1 按 Jaccard 降序、C2 按交集数降序，`C1_TOP_K_PER_DOMAIN = 15`，C1/C2 共用）
> - **S4 极限（>300）**：Top-K 收紧到每域每类型 10（`SCALE_S4_TOP_K`），且处方批量聚合
>   （maintain/fix-frontmatter/fix-or-archive/slim 同型合并、add-antitrigger 按 Skill 聚合；
>   merge/boundary/schedule 保持逐对）
>
> 域映射取自 `.medic/_medic_classify.json` 的 `domain_final`（LLM 回填后）；无映射时退化为全同域比对（`load_domain_map`）。
> 效果：461 个 Skill 场景下候选收敛到「域数 × Top-K × 2」量级（约几百组），而非 O(N²) 的十万级。
> 实测：150+ Skill（S3）候选 294 组；361 Skill（S4）候选 200 组、处方经聚合从数百条降到 184 条。
