...
1. 读取 `.medic/_medic_classify.json`（`run.py categorize --save` 产出）的静态初值（`domain_hint` / `interaction` / `lifecycle`）
2. **功能域 = LLM 语义判定（开放域）**：
...
**回填必须写回 `.medic/_medic_classify.json`**（用 IDE Write 工具覆盖更新该 JSON 的 `domain_final` 字段），
